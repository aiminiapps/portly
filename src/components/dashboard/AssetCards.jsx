...
import { FiArrowUpRight, FiArrowDownRight, FiZap, FiMoreHorizontal } from 'react-icons/fi';
...
export default function AssetCards({ assets }) {
  ...
  if (!assets || assets.length === 0) {
    return (
      <div className="rounded-3xl border border-white/5 bg-[#121214]/40 p-12 text-center">
        <p className="text-[#9CA3AF]">No assets found in your portfolio</p>
      </div>
    );
  }

  // Optimized Sparkline (Minimalist)
  const getSparklineOption = (data, isPositive) => ({
    ...
    series: [{
      type: 'line',
      data: data?.length > 0 ? data : [0, 0, 0, 0, 0, 0, 0], // Fallback data to prevent crash
      smooth: true,
      symbol: 'none',
      lineStyle: {
        width: 2,
        color: isPositive ? '#4ADE80' : '#EF4444', // Green or Red
        opacity: 0.8
      },
      areaStyle: {
        color: {
    ...
    try {
      const response = await fetch('/api/agent', {
        ...
        body: JSON.stringify({
          messages: [{
            role: "user",
            content: `Analyze ${asset.symbol}: Value $${asset.value?.toFixed(2)}, Change ${asset.change24h?.toFixed(2)}%. Give 3 short bullet points.`
          }]
        })
      });
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {assets.map((asset, index) => {
          // SAFE FALLBACKS (Fixes the crash)
          ...
          return (
            <motion.div
              ...
              className="group relative flex flex-col justify-between p-6 rounded-3xl border border-white/5 bg-[#121214]/60 backdrop-blur-xl hover:bg-[#121214]/80 transition-colors"
            >
              {/* Card Header */}
              <div className="flex items-start justify-between mb-6">
                <div className="flex items-center gap-4">
                  {/* Icon Container */}
                  <div className="relative w-12 h-12 rounded-2xl bg-white/5 border border-white/5 flex items-center justify-center overflow-hidden">
                     {asset.logo ? (
              ...
              {/* Sparkline (Subtle) */}
              <div className="h-10 w-full opacity-50 group-hover:opacity-100 transition-opacity mb-4">
                 <ReactECharts
              ...
              <div className="pt-4 border-t border-white/5">
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-white/5 hover:bg-[#8B5CF6]/20 hover:text-[#8B5CF6] transition-all text-xs font-medium text-white/60"
                >
                  ...
                  <span>{loadingInsights[asset.symbol] ? 'Analyzing...' : 'AI Analysis'}</span>
                </button>
              </div>
              ...
              <AnimatePresence>
                {showAIInsights[asset.symbol] && aiInsights[asset.symbol] && (
                  ...
                  >
                    <div className="mt-3 p-3 rounded-xl bg-[#8B5CF6]/10 border border-[#8B5CF6]/20">
                      <p className="text-[11px] leading-relaxed text-[#E5E7EB] whitespace-pre-line">
